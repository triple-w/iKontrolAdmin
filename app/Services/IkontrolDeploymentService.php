...
final class IkontrolAdminProvision extends BaseCommand {
 ...
 public function run(array $params){if(count($params)!==2||!filter_var($params[1],FILTER_VALIDATE_EMAIL))throw new \RuntimeException('Datos de administrador inválidos.');$password=rtrim((string)fgets(STDIN),"\r\n");if(strlen($password)<12)throw new \RuntimeException('La contraseña debe tener al menos 12 caracteres.');$email=strtolower($params[1]);$users=model('App\Models\Users_model');$existing=$users->get_one_where(['email'=>$email,'deleted'=>0]);$db=\Config\Database::connect();$db->transStart();if($existing&&$existing->id){if($existing->user_type!=='staff'||!(int)$existing->is_admin||(int)$existing->role_id!==0)throw new \RuntimeException('El usuario existente no es un administrador reparable.');$userId=(int)$existing->id;$operation='ADMIN_REPAIRED';}else{$parts=preg_split('/\s+/',trim($params[0]),2);$userId=$users->ci_save(['first_name'=>$parts[0],'last_name'=>$parts[1]??'','user_type'=>'staff','is_admin'=>1,'role_id'=>0,'email'=>$email,'password'=>password_hash($password,PASSWORD_DEFAULT),'status'=>'active','client_id'=>0,'is_primary_contact'=>0,'job_title'=>'Admin','disable_login'=>0,'gender'=>'male','language'=>'','enable_web_notification'=>1,'enable_email_notification'=>1,'created_at'=>date('Y-m-d H:i:s'),'requested_account_removal'=>0,'deleted'=>0]);$operation='ADMIN_CREATED';}unset($password);if(!$userId||!$db->tableExists('team_member_job_info'))throw new \RuntimeException('No fue posible preparar el administrador completo.');$job=$users->get_job_info($userId);if(!$job||!$job->user_id){$saved=$users->save_job_info(['user_id'=>$userId,'salary'=>0,'salary_term'=>'','date_of_hire'=>date('Y-m-d')]);if(!$saved)throw new \RuntimeException('No fue posible crear team_member_job_info.');}$db->transComplete();if(!$db->transStatus())throw new \RuntimeException('No fue posible crear o reparar el administrador completo.');CLI::write($operation,'green');}
}
PHP);
...
PHP);
        $baselineExport=var_export(array_merge(...array_values((array)config('ikontrol.deployment.settings_baseline',[]))),true);
        $settingsCommand=<<<'PHP'
<?php
namespace App\Commands;
use CodeIgniter\CLI\{BaseCommand,CLI};
final class IkontrolSettingsBaseline extends BaseCommand {
 ...
 private const BASELINE=__BASELINE__;
         public function run(array$p){
          if($p)throw new \RuntimeException('Este comando no acepta argumentos.');
          try{$db=\Config\Database::connect();if(!$db->tableExists('settings')){$this->emit(['status'=>'FAILED','reason'=>'SETTING_TABLE_MISSING']);return;}$table=$db->table('settings');$created=[];$corrected=[];$preserved=[];
           foreach(self::BASELINE as$name=>$default){$default=$this->normalize($default);$query=$table->select('setting_name,setting_value,type,deleted')->where('setting_name',$name)->get();if($query===false){$this->emit(['status'=>'FAILED','reason'=>'SQL_QUERY_FAILED','operation'=>'read','setting'=>$name]);return;}$row=$query->getRowArray();
            if(!$row){$insert=$table->insert(['setting_name'=>$name,'setting_value'=>$default,'type'=>'app','deleted'=>0]);if($insert===false){$this->emit(['status'=>'FAILED','reason'=>'SQL_WRITE_FAILED','operation'=>'insert','setting'=>$name]);return;}$created[]=$name;continue;}
            $value=$this->normalize($row['setting_value']??'');$replacement=null;if(in_array($name,['site_logo','favicon'],true)&&in_array(trim($value),['b:0;','N;'],true))$replacement='';if($name==='item_purchase_code'&&trim($value)==='')$replacement=$default;if($name==='timezone'&&trim($value)==='')$replacement=$default;if($replacement!==null||$value!==($row['setting_value']??'' )||((int)($row['deleted']??0)!==0)){$update=$table->where('setting_name',$name)->update(['setting_value'=>$replacement??$value,'type'=>$row['type']??'app','deleted'=>0]);if($update===false){$this->emit(['status'=>'FAILED','reason'=>'SQL_WRITE_FAILED','operation'=>'update','setting'=>$name]);return;}$corrected[]=$name;}else{$preserved[]=$name;}
           }$check=$this->check($db);$this->emit(['status'=>$check['status'],'reason'=>$check['reason'],'created'=>$created,'corrected'=>$corrected,'preserved'=>$preserved,'checks'=>$check['checks']]);
          }catch(\Throwable$e){$this->emit(['status'=>'FAILED','reason'=>'SQL_QUERY_FAILED','error'=>get_class($e),'message'=>$e->getMessage()]);}
         }
         private function normalize($value){if($value===null)return '';if(is_bool($value))return $value?'1':'0';if(is_int($value)||is_float($value))return (string)$value;if(is_string($value))return $value;throw new \RuntimeException('El setting contiene un valor no escalar.');}
         private function check($db){$query=$db->table('settings')->select('setting_name,setting_value')->where('deleted',0)->whereIn('setting_name',array_keys(self::BASELINE))->get();if($query===false) return ['status'=>'FAILED','reason'=>'SQL_QUERY_FAILED','checks'=>[]];$values=[];foreach($query->getResultArray() as$r)$values[$r['setting_name']]=$this->normalize($r['setting_value']??'');$checks=[];$reason=null;foreach(self::BASELINE as$name=>$default){$exists=array_key_exists($name,$values);$valid=$exists;if(in_array($name,['site_logo','favicon'],true))$valid=$exists&&!in_array(trim($values[$name]??''),['b:0;','N;'],true);if(in_array($name,['timezone','item_purchase_code'],true))$valid=$exists&&trim($values[$name]??'')!=='';$checks[$name]=['present'=>$exists,'valid'=>$valid,'non_null'=>$exists&&$values[$name]!==null];if(!$valid&&$reason===null)$reason=!$exists?'SETTING_MISSING':($name==='site_logo'?'SITE_LOGO_INVALID':($name==='favicon'?'FAVICON_INVALID':($name==='item_purchase_code'?'ITEM_PURCHASE_CODE_MISSING':'SETTING_INVALID')));}$status=$reason===null?'READY':'FAILED';return compact('status','reason','checks');}
 private function emit(array$d){CLI::write("IKONTROL_JSON_BEGIN\n".json_encode($d,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)."\nIKONTROL_JSON_END");}
}
PHP;
        File::put($directory.DIRECTORY_SEPARATOR.'IkontrolSettingsBaseline.php',str_replace('__BASELINE__',$baselineExport,$settingsCommand));
        // Replace the legacy table-only dashboard probe with the functional baseline contract.
        $dashboardRequiredExport=var_export(array_keys(array_merge(...array_values((array)config('ikontrol.deployment.settings_baseline',[])))),true);
